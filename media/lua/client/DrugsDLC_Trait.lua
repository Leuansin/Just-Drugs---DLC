...
DrugsDLC.TraitDrogadicto = function()
    local TraitAdicto = TraitFactory.addTrait("Drogadicto", "Drug  Addict", -8,
        "You like to confront problems in a different way. But your body will ask for more every 12 hours. You can eventually outgrow your addiction if you don't consume for 21 days in a row.",
        false, false);
...
DrugsDLC.TraitWalterWhite = function()
    local TraitWalterWhite = TraitFactory.addTrait("WalterWhite", "Drug Lord", 4,
        "You were always smarter than the rest. ¡You were the one making & selling the drugs to them!", false, false);
...
DrugsDLC.GivePlayerItem = function()
    ...
    if player:HasTrait("Drogadicto") then
        player:getInventory():AddItem("DrugsDLC.DrugPipe") -- Añadir el item al inventario
...
DrugsDLC.IncreaseEffectsEveryHour = function()
    ...
    if player:HasTrait("Drogadicto") then
        ...
        local felicidadLevel = bodyDamage:getUnhappinessLevel()

        bodyDamage:setBoredomLevel(boredomLevel + 10) -- Aumentar aburrimiento
        bodyDamage():setUnhappinessLevel(felicidadLevel + 10)
    end
end
...
DrugsDLC.AbstinenceSystem = function()
    ...
    if player:HasTrait("Drogadicto") then
        ...
        local hasDrugs = player:getInventory():contains("DrugsDLC.Cocaine") or
            ...
            player:getInventory():contains("DrugsDLC.Acid")
        ...
        if modData.abstinenceTimer >= 12 then
            ...
            -- Dolor generalizado
            bodyDamage:setContinualPainIncrease(7) -- Aumentar dolor
            stats:setStress(8)                     -- Aumentar estrés
            stats:setFatigue(8)                     -- Aumentar fatiga
