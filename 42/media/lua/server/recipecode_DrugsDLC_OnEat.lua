...
-- Variable para el error de un solo uso
local hasLoggedPlayerError = false
...
        DayTime = {
            Inside = {
                Whispers = {"What was that?", "I heard something...", "I'm not alone."},
                Confusing = {"Where am I?", "This isn't real.", "What is happening?"},
                Panic = {"No, no, no!", "I have to get out of here!", "They're going to get me!"}
            },
            Outside = {
                Whispers = {"Shhh...", "Someone is watching me.", "Is anyone there?"},
                Confusing = {"The sky is moving...", "These colors...", "Why is everything glowing?"},
                Panic = {"They're in the trees!", "Runnn!", "Don't follow me!"}
            }
        },
        NightTime = {
            Inside = {
                Whispers = {"(Unintelligible whispers)", "Behind you...", "Don't look."},
                Confusing = {"The walls are closing in.", "I can't... breathe.", "Is that blood?"},
                Panic = {"Leave me alone!", "It's in the house!", "I'M GOING TO DIE!"}
            },
            Outside = {
                Whispers = {"We are watching you.", "Run...", "Come with us..."},
                Confusing = {"The moon... it's talking to me.", "I can't feel my legs.", "Everything is spinning."},
                Panic = {"DON'T TOUCH ME!", "SHADOWS, SHADOWS EVERYWHERE!", "HELP!"}
            }
        }
    },
    AudioReactions = {
        fakethump = {"What was that noise?", "Hello?", "Someone's upstairs..."},
        fakethumpsqueak = {"A door slam!", "Who closed it?", "Shit, shit..."},
        fakedooropen = {"Who opened the door?", "No... don't come closer."},
        fakewindowSmash = {"A window!", "They're breaking in!", "I have to hide!"},
        fakebush = {"Something is moving out there...", "It's... it's just the wind, right?"},
        fakecrackwood = {"A branch... or a footstep?", "Shhh, quiet."}
    },
    VisualReactions = {
        FakeZombie = {"THERE'S ONE!", "How did it get in?", "It's not real, it's not real!", "Get away from me!"},
        FakeInfected = {"Did... did I get bit?", "I feel... cold.", "No, I can't be infected.", "I'm turning..."}
    }
}
...
    if not player then
        if not hasLoggedPlayerError then
            print("DrugsDLC: ERROR - Player not found! (This message will only appear once)")
...
        if DrugsDLC.DebugMode then print("[DrugsDLC] Audio Hallucination") end
...
            if DrugsDLC.DebugMode then print("[DrugsDLC] Hallucination: Fake Zombie") end
            DrugsDLC.spawnFakeZombie(player)
        elseif specific == "FakeInfected" then
            if DrugsDLC.DebugMode then print("[DrugsDLC] Hallucination: Fake Infection") end
            DrugsDLC.doFakeInfect(player)
        end
        
    elseif hallucinationType == "Line" then
        if DrugsDLC.DebugMode then print("[DrugsDLC] Hallucination: Voice Line") end
        DrugsDLC.sayHallucinationLine(player, location)
        reacted = true
...
local function handleAddiction(drugType, addictionPoints, character)
    local player = character or getSafePlayer()
    if not player then return end
    
    local modData = player:getModData()
    
...
            player:Say(getText("IGUI_AlreadyAddicted") or "I need my fix...")
        else
            player:getTraits():add("Drogadicto")
            modData.drugAddictionCounter = addictionThreshold
            player:Say(getText("IGUI_BecameAddicted") or "I think... I'm hooked...")
            
            local stats = player:getStats()
...
        if progress >= 75 then
            player:Say(getText("IGUI_AddictionWarning") or "I'm becoming dependent...")
        elseif progress >= 50 then
            player:Say(getText("IGUI_AddictionBuilding") or "This feels too good...")
        end
    end
end
...
    handleAddiction("general", 1, character)
end

-- Cocaína
OnEat_HasTakenCocaine = function(player, item, character)
    handleAddiction("cocaine", 2, character)
...
    handleAddiction("fentanyl", 3, character)
    local stats = character:getStats()
...
        character:Say(getText("IGUI_FentanylOverdose") or "That was too strong...")
    end
end

-- Xanax
OnEat_HasTakenXanax = function(player, item, character)
    handleAddiction("xanax", 2, character)
    local stats = character:getStats()
...
                modData.drugAddictionCounter = 0
                player:Say(getText("IGUI_OvercameAddiction") or "I think... I'm finally clean!")
            end
        end
    end
    
...
    end
    
    print("DrugsDLC: Addiction and Hallucination System initialized for Build 42")
end
...
function OnEat_Tripping(player, item, character)
    local player = character or getSafePlayer()
    if not player then return end
    
    local stats = player:getStats()
...
function OnEat_TrippingHardcore(player, item, character)
    local player = character or getSafePlayer()
    if not player then return end
    
    local stats = player:getStats()
...
            })
            player:Say("I can see the universe!")
            
        elseif randomNumber <= goodChance + badChance then
            -- Viaje malo hardcore
            applyStatModifiersB42(stats, bodyDamage, {
...
            })
            player:Say("The walls are breathing...")
            
        else
...
t        modData.acidHallucinationDose = 0
    end
...
Two      modData.hallucinationEnds = getGameTime():getWorldAgeHours() + ZombRandFloat(2.0, 4.0)
        
        -- Reiniciar el contador de dosis
        modData.acidHallucinationDose = 0 
        
        player:Say("I feel... like everything is melting...")
        
        -- Forzar un aumento de pánico y estrés MÁS FUERTE
Click      DrugsDLC.applyMood(25, 0.15, true, false, player)
nd
    end
    
end
...
    handleAddiction("shrooms", 1, character)
    
...
Image          modData.shroomsAddictionCounter = shroomsThreshold
            character:Say("I need the visions...")
        end
    end

    -- 2. Ejecutar el "viaje" (bueno/malo/horrible) (de tu script OnEat)
    OnEat_Tripping(player, item, character) -- Llama a la función de viaje normal

    -- 3. ¡NUEVA LÓGICA! Activar sistema de alucinaciones TAW
    if not modData.shroomHallucinationDose then
  A     modData.shroomHallucinationDose = 0
    end
    modData.shroomHallucinationDose = modData.shroomHallucinationDose + 1
...
        modData.shroomHallucinationDose = 0 
      t 
        player:Say("Oh... oh no... this is too much...")
Ah       
...
config        modData.shroomHallucinationDose = 0 -- Reiniciar la dosis
        player:Say("It's... it's wearing off. Finally.")
        
...
    Player    if fakeZombie then
            fakeZombie:removeFromWorld()
Ah           fakeZombie:removeFromSquare()
            fakeZombie = nil
            Events.OnTick.Remove(DrugsDLC.canPlayerSee)
        end
        return
...
Player    DrugsDLC.SchizoPersonality(player)
    
    -- 2. Ejecutar alucinaciones principales (visuales, auditivas, líneas)
Si    local location = player:getBuilding() and "Inside" or "Outside"
  T DrugsDLC.HandleHallucination(player, location)
end
